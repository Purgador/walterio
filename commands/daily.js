...
module.exports = async (client, message, args, ops) => {
  const lang = ops.lang.commands.daily
if(cooldown.has(`${message.author.id}.daily`)){
  let tiempo = await cooldown.get(`${message.author.id}.daily`);
    if (Date.now() < tiempo) {
  ...
  await cooldown.set(`${message.author.id}.daily`, Date.now() + 86400000)
